...
def generate_initial_price(stock_code, stock_name, price_list, product_num):
    ...
    if len(price_list) == 1:
        ...
        # 3数平均数
        target_mean = np.mean([price_median, custom_mean, sorted_price_list[1], sorted_price_list[2]])
        # target_mean = np.mean([price_median, price_weight_mean, custom_mean])

        logger.info('%s, %s, %s, (%s), (%s), (%s)' % (
            stock_code, stock_name, (str(round(price_std, 2)) + ', 宽') if price_std > 0.2 else str(round(price_std, 2)),
            ','.join(str(i) for i in sorted(price_list, reverse=True)),
            ','.join(str(round(i, 2)) for i in [price_median, custom_mean, sorted_price_list[1], sorted_price_list[2]]),
            round(target_mean, 2)
        ))
...
if __name__ == '__main__':
    ...
    # target_price_num += generate_initial_price('688658', '悦康药业: 24 EPS: 1.1', [i[1] for i in all_price_list], 360)
    # target_price_num += generate_initial_price('688679', '通源环境: 12 EPS: 0.89', [i[2] for i in all_price_list], 360)
